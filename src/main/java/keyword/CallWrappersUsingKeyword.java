	...
	public void getAndCallKeyword(String fileName) throws Exception{
		...
		for (int i = 1; i <= sh.getLastRowNum(); i++) {
			...
			try {
				...
				sh.getRow(i).getCell(4).setCellType(Cell.CELL_TYPE_STRING);
				data = sh.getRow(i).getCell(4).getStringCellValue();
				
				sh.getRow(i).getCell(5).setCellType(Cell.CELL_TYPE_STRING);
				result = sh.getRow(i).getCell(5).getStringCellValue();
